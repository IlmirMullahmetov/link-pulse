    ...
    <>
      {sites?.length === 0 ? (
        ...
        <div className='flex flex-col gap-4'>
          {sites?.map((site) => (
            <SiteCard site={site} key={site.id} onOpenModal={(action, site) => setModal({ action, site })} />
          ))}
          <Modal isOpen={!!modal} onClose={() => setModal(null)}>
            {modal?.action === 'edit' && <EditModalContent onClose={() => setModal(null)} site={modal.site} />}
            {modal?.action === 'delete' && <DeleteModalContent onClose={() => setModal(null)} site={modal.site} />}
          </Modal>
        </div>
      )}
    </>
